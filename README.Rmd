...
*PCAtools* provides functions for data exploration via PCA, and allows the user to generate publication-ready figures. PCA is performed via *BiocSingular* [@Lun] - users can also identify optimal number of principal components via different metrics, such as the elbow method, which has relevance for data reduction in single-cell RNA-seq (scRNA-seq) and high dimensional mass cytometry data.
...
Finding the correct number of PCs is a difficult task and is akin to finding the 'correct'
number of clusters in a dataset - there is no single correct answer.
